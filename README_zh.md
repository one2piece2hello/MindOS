...
<details>
<summary><strong>Agent 完整工具集（20 个）</strong></summary>

| 工具 | 说明 |
|------|------|
| `mindos_bootstrap` | 一次性加载启动上下文（INSTRUCTION + README） |
| `mindos_list_files` | 知识库完整文件树 |
| `mindos_read_file` | 读取文件内容（支持分页） |
| `mindos_write_file` | 覆写文件（受保护文件被拦截） |
| `mindos_create_file` | 创建新 .md/.csv 文件 |
| `mindos_delete_file` | 删除文件（受保护文件被拦截） |
| `mindos_rename_file` | 原地重命名文件 |
| `mindos_move_file` | 移动文件 + 报告受影响的反向链接 |
| `mindos_search_notes` | 全文搜索（支持范围/类型/日期筛选） |
| `mindos_get_recent` | 最近修改的文件 |
| `mindos_get_backlinks` | 查找所有引用指定文件的文件 |
| `mindos_get_history` | 文件的 Git 提交历史 |
| `mindos_get_file_at_version` | 读取指定 Git 提交版本的文件内容 |
| `mindos_append_csv` | 向 CSV 文件追加行 |
| `mindos_read_lines` | 按行号读取文件 |
| `mindos_insert_lines` | 在指定位置插入行 |
| `mindos_update_lines` | 替换行范围 |
| `mindos_append_to_file` | 向文件末尾追加内容 |
| `mindos_insert_after_heading` | 在标题后插入内容 |
| `mindos_update_section` | 替换 Markdown 章节 |
...
| Claude Code | ✅ | 通过 `claude mcp add` 接入 |
| CodeBuddy | ✅ | 在设置中配置 MCP |
| Cursor | ✅ | 在设置中配置 MCP |
| Windsurf | ✅ | 在设置中配置 MCP |
| Cline | ✅ | 在设置中配置 MCP |
| Trae | ✅ | 在设置中配置 MCP |

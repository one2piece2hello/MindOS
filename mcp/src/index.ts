...
function isRootProtected(filePath: string): boolean {
  const normalized = path.normalize(filePath);
  return ROOT_PROTECTED_FILES.has(normalized);
}

function assertNotProtected(filePath: string, operation: string): void {
...
interface SearchOptions {
  ...
  modified_after?: string; // ISO date string, e.g. "2025-01-01"
}

function searchFiles(query: string, opts: SearchOptions = {}): SearchResult[] {
...
function renameFile(oldPath: string, newName: string): string {
...
function findBacklinks(targetPath: string): BacklinkEntry[] {
...
function gitLog(filePath: string, limit: number): Array<{ hash: string; date: string; message: string; author: string }> {
  const resolved = resolveSafe(filePath);
  const output = execSync(
    `git log --follow --format="%H%x00%aI%x00%s%x00%an" -n ${limit} -- "${resolved}"`,
    { cwd: MIND_ROOT, encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }
  ).trim();
  if (!output) return [];
  return output.split("\n").map(line => {
...
function gitShowFile(filePath: string, commitHash: string): string {
  const resolved = resolveSafe(filePath);
...
function moveFile(fromPath: string, toPath: string): { newPath: string; affectedFiles: string[] } {
...
function ensureAuditFile(filePath: string, title: string): void {
...
function logOp(tool: string, params: Record<string, unknown>, result: "ok" | "error", message?: string): void {
  try {
...
function logDiff(tool: string, filePath: string, before: string, after: string): void {
  try {
...
Error Handling:
  ...
  async ({ path: filePath }) => {
    try {
      ...
      let before = "";
      try { before = readFile(filePath); } catch { /* ignore */ }
      deleteFile(filePath);
...
Error: Throws if heading not found.`,
  ...
  async ({ path: filePath, heading, content }) => {
    try {
      ...
      if (before !== after) logDiff("mindos_update_section", filePath, before, after);
  ...
  async ({ from_path, to_path }) => {
    try {
      ...
      if (affectedFiles.length > 0) {
        ...
        for (const f of affectedFiles) lines.push(`  - ${f}`);
      } else {
